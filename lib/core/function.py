...
def select_random_chunk(image, chunk_size):
    print("Select Random Chunk, Image Shape: ", image.shape)
    B, C, H, W = image.shape
    chunk_h, chunk_w = chunk_size

    print("chunk_h: ", chunk_h, "H: ", H)
    print("chunk_w: ", chunk_w, "W: ", W)

    if chunk_h > H or chunk_w > W: 
        raise ValueError("Chunk size must be smaller than the image dimensions")

    start_h = np.random.randint(0, H - chunk_h + 1)
    start_w = np.random.randint(0, W - chunk_w + 1)
    ...
    return (start_h, start_w), (chunk_h, chunk_w), (x1, x2, y1, y2)

def apply_mask(images, chunk_coords):
    print("Apply Mask, Images Shape: ", images.shape)
    B, C, H, W = images.shape
    (start_h, start_w), (chunk_h, chunk_w), (x1, x2, y1, y2) = chunk_coords

    masked_images = torch.zeros_like(images)
    for i in range(B):
        image_np = images[i].permute(1, 2, 0).cpu().numpy()
        masked_image = np.zeros((H, W, C), dtype=np.uint8)
        masked_image[start_h:start_h + chunk_h, start_w:start_w + chunk_w, :] = image_np[start_h:start_h + chunk_h, start_w:start_w + chunk_w, :]
        masked_images[i] = torch.tensor(masked_image).permute(2, 0, 1)
    return masked_images

# Function to save a selected chunk as an image
def save_masked_image(masked_images, chunk_coords, save_path='chunks/masked_image'):
    print(f"\nPre-Pre-Pre-Repeat:\nDimension: {masked_images.dim()}\nShape: {masked_images.shape}")
    
    for i, masked_image in enumerate(masked_images):
        print(f"\nPre-Pre-Repeat:\nDimension: {masked_image.dim()}\nShape: {masked_image.shape}")
        '''
        # Ensure there is a channel dimension in the correct place
        if masked_image.dim() == 3 and masked_image.shape[1] != 3:  # Assuming channels should be at the second position
            masked_image = masked_image.unsqueeze(1)  # Insert a singleton dimension at position 1 for channels
            print(f"\nPre-Repeat:\nDimension: {masked_image.dim()}\nShape: {masked_image.shape}")

            masked_image = masked_image.repeat(1, 3, 1, 1) 
            print(f"\nDimension: {masked_image.dim()}\nShape: {masked_image.shape}")'''
        # Convert the tensor to a NumPy array and permute dimensions to HWC
        masked_image_np = masked_images.permute(1, 2, 0).cpu().numpy()
        
        
        # Define the save path for each image
        individual_save_path = f'{save_path}_{i}.jpg'
        
        # Save the image
        cv2.imwrite(individual_save_path, cv2.cvtColor(masked_image_np, cv2.COLOR_RGB2BGR))

def load_images_as_chunks(directory_path):
    chunk_tensors = []
    ...
    for filename in sorted(os.listdir(directory_path)):
        ...
        if file_path.endswith('.jpg'):
            ...
            # Load the image
            chunk = cv2.imread(file_path)
            if chunk is None:
                print(f"Failed to load image at {file_path}")
                continue
            else:
                print("Image loaded successfully.")
                print("Shape of loaded image:", chunk.shape)

            chunk = cv2.cvtColor(chunk, cv2.COLOR_BGR2RGB)
            print("Converted image shape:", chunk.shape)

            # Convert to PyTorch tensor
            chunk_tensor = torch.tensor(chunk).permute(2, 0, 1).float()
            print("The chunk tensor:", chunk_tensor.shape)

            chunk_tensors.append(chunk_tensor)

    if not chunk_tensors:
        raise ValueError("No images loaded from the directory.")
    
    # Stack tensors to form a batch
    batch_tensor = torch.stack(chunk_tensors)
    print("Batch tensor shape:", batch_tensor.shape)
    
    return batch_tensor

def plot_and_save_saliency_maps(saliency_maps, save_directory):
